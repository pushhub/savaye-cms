...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const populateOpt = populate || Link.associations
  ...
  add: async (values) => {
    ...
    return Link.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Link.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Link.attributes).reduce((acc, curr) => {
      switch (Link.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
